...
namespace AlfaPeople.KingKhalidFoundation.Web.Controllers
{
    ...
    : Url.Content("~/assets/img/default-avatar.png"); // الصورة الافتراضية
        ...
        public ActionResult dashboard()
        {
            ...
            if (currentuser.AspNetRoles.Any(r => r.Name == "Corporation"))
            {
                ...
                if (repository.GetQuery<FrontendUser>(f => f.FK_AspUser == userid && f.CorporateApplicationForms.Any(g => g.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName == "Accepted")).ToList().Count != 0)
                {
                    ...
                    //4-2-2026
                    _dashboard.WorkshopHasPostImpact = new Dictionary<Guid, bool>();
                    _dashboard.WorkshopPostImpactLastSubmit = new Dictionary<Guid, DateTime?>();
                    //5-2-2026
                    _dashboard.WorkshopHasPostImpactRequest = new Dictionary<Guid, bool>();
                ...
                (f.IsActive == true && !f.IsDeleted && (f.EndDate >= dateNow) && f.IncubationType.NameEN == "Incubation" &&
                    ...
                    foreach (var workshop in _dashboard._workShop)
                    {
                        ...
                        var hasAcceptedBLTransaction = repository.GetQuery<IncubationWorkshopBLTransactionsValue>()
                            ...
                            //4-2-2026
                               j.IncubationWorkshopBLTrans.TransTypeID == BaselineTransTypeId &&
                    ...
                    //foreach (var workshop in _dashboard._workShop)
                    //{
                    //    var postImpactValues = repository.GetQuery<IncubationWorkshopBLTransactionsValue>()
                    //        .Where(v =>
                    //            v.FrontendUserID == frontEnd &&
                    //            v.IncubationWorkshopBLTrans.IncubationWorkshopID == workshop.IncubationWorkshopID &&
                    //            v.IncubationWorkshopBLTrans.TransTypeID == PostImpactTransTypeId)
                    //        .ToList();

                    //    bool hasFilled = postImpactValues.Any();
                    //    _dashboard.WorkshopHasPostImpact[workshop.IncubationWorkshopID] = hasFilled;

                    //    var lastSubmit = postImpactValues
                    //        .Where(x => x.SubmissionDate != null)
                    //        .OrderByDescending(x => x.SubmissionDate)
                    //        .Select(x => (DateTime?)x.SubmissionDate)
                    //        .FirstOrDefault();

                    //    _dashboard.WorkshopPostImpactLastSubmit[workshop.IncubationWorkshopID] = lastSubmit;
                    //}
                    // ورش قياس الأثر (منفصلة عن ورش التقديم)
                    var postImpactWorkshopIds = repository.GetQuery<ProjectImpactEvaluationRequest>()
                        .Where(r => r.FrontendUserId == frontEnd)
                        .Select(r => r.ProjectImpactEvaluation.IncubationWorkshopID)
                        .Distinct()
                        .ToList();

                    _dashboard._workShopPostImpact = repository.GetQuery<IncubationWorkshop>()
                        .Where(w => postImpactWorkshopIds.Contains(w.IncubationWorkshopID) && !w.IsDeleted)
                        .ToList();
                    foreach (var workshop in _dashboard._workShopPostImpact)
                    {
                    
                        _dashboard.WorkshopHasPostImpactRequest[workshop.IncubationWorkshopID] = true;
        ...
        public ActionResult IncWSBaselineAppForm(Guid id)
        {
            ...
            var transactionType = repository.GetQuery<IncubationWorkshopBLTransactionsType>()
                .Include(t => t.IncubationWorkshopBLTrans.Select(trans => trans.Options))
                .Include(t => t.IncubationWorkshopBLTrans.Select(trans => trans.IncubationWSControlsType))
                .FirstOrDefault(t => t.TransTypeID == BaselineTransTypeId &&
                     t.IncubationWorkshopBLTrans.Any(trans => trans.IncubationWorkshopID == id));
            ...
            if (transactionType != null)
            {
                ...
                var incIL = repository.GetQuery<IncubationWorkshopBLTransactionsValue>(v =>
                    ...
                    v.IncubationWorkshopBLTrans.IncubationWorkshopID == id
                    &&
                     v.IncubationWorkshopBLTrans.TransTypeID == BaselineTransTypeId)
                    .Include(v => v.IncubationWorkshopBLTrans)
                    .Include(v => v.OptionValues)
                    .Include(v => v.IncubationWorkshopBLTransValStatus)
                    .ToList();
        ...
        public ActionResult IncWSBaselineAppForm(IncubationWorkshopBLTransactionsType model, string WorkshopName, Guid WorkshopID, string CheckboxData)
        {
            ...
            try
            {
                ...
                .Include(t => t.IncubationWorkshopBLTrans.Select(trans => trans.IncubationWSControlsType))
                .FirstOrDefault(/*t => t.TransTypeName == "IncubationWorkshopBaselineTransactions" &&*/
                    t => t.TransTypeID == BaselineTransTypeId &&
        ...
        public ActionResult IncubationWorkshopJoin(Guid id)
        {
            ...
            try
            {
                ...
                var workshopTransaction = CurrUser.IncubationWSBLTransactionsValue
                    ...
                    t.IncubationWorkshopBLTrans.TransTypeID == BaselineTransTypeId);
        ...
        private string SavePostImpactFile(HttpPostedFileBase file, Guid workshopId, Guid transValueId)
        {
            ...
            var allowedExt = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".png", ".jpg", ".jpeg"
            ...
            var relativeFolder = $"~/Uploads/PostImpact/{workshopId}/{transValueId}";
            var physicalFolder = Server.MapPath(relativeFolder);
            ...
            var physicalPath = Path.Combine(physicalFolder, savedName);

            file.SaveAs(physicalPath);

            //
            var relativeUrl = VirtualPathUtility.ToAbsolute($"{relativeFolder}/{savedName}");

            return relativeUrl;
        }

        [HttpGet]
        public ActionResult IncWSPostImpactForm(Guid id)
        {
            var frontEnd = Guid.Parse(User.Identity.GetUserId());
            ViewBag.frontEnd = frontEnd;
            ...
            bool hasRequest = repository.GetQuery<ProjectImpactEvaluationRequest>()
                .Any(r => r.FrontendUserId == frontEnd && r.ProjectImpactEvaluation.IncubationWorkshopID == id);

            if (!hasRequest)
                return RedirectToAction("dashboard", new { Msg = "لا يوجد طلب تقييم أثر لهذه الورشة." });

            // جلب نوع المعاملات PostImpact
            var transactionType = repository.GetQuery<IncubationWorkshopBLTransactionsType>()
                .Include(t => t.IncubationWorkshopBLTrans.Select(trans => trans.Options))
                .Include(t => t.IncubationWorkshopBLTrans.Select(trans => trans.IncubationWSControlsType))
                .FirstOrDefault(t => t.TransTypeID == PostImpactTransTypeId
                    && t.IncubationWorkshopBLTrans.Any(trans => trans.IncubationWorkshopID == id));

            if (transactionType != null)
            {
                // عرض الحقول المسموحة فقط
                transactionType.IncubationWorkshopBLTrans = transactionType.IncubationWorkshopBLTrans
                    .Where(trans => trans.IncubationWorkshopID == id && trans.ViewList_Display == "true")
                    .ToList();

                // جلب القيم السابقة (PostImpact فقط)
                ...
                if (values.Any())
                {
                    ViewBag.PostImpactStatus = values.FirstOrDefault()?.IncubationWorkshopBLTransValStatus?.NameEN;
                    ViewBag.Msg = values.FirstOrDefault()?.Feadback;
                }

                // ربط كل Trans بالقيم الخاصة به
                foreach (var trans in transactionType.IncubationWorkshopBLTrans)
                {
                    trans.IncubationWSBLTransactionsValue = values
                        .Where(v => v.TransID == trans.TransID)
                        .ToList();

                    // إذا ما في قيمة، نضيف واحدة فارغة حتى يظهر الحقل
                    if (!trans.IncubationWSBLTransactionsValue.Any())
                    {
                        trans.IncubationWSBLTransactionsValue.Add(new IncubationWorkshopBLTransactionsValue
                        {
                            ...
                            SubmissionDate = DateTime.UtcNow,
                            IncubationWorkshopBLTransValStatusID =
                                repository.Get<IncubationWorkshopBLTransValStatus>(s => s.NameEN == "Pending")
                                .FirstOrDefault()?.IncubationWorkshopBLTransValStatusID ?? Guid.Empty
                        });
                    }
                }
        ...
        [HttpPost]
        public ActionResult IncWSPostImpactForm(IncubationWorkshopBLTransactionsType model, string WorkshopName, Guid WorkshopID, string CheckboxData)
        {
            ViewBag.lang = CultureHelper.CurrentCulture;
            var frontEnd = Guid.Parse(User.Identity.GetUserId());
            try
            {
                ...
                if (transType == null)
                {
                    TempData["Msg1"] = "لا يوجد نموذج Post Impact لهذه الورشة.";
                    return RedirectToAction("dashboard");
                }

                
                // 1) CheckboxData (Multi-select)
                ...
                if (!string.IsNullOrEmpty(CheckboxData))
                {
                    ...
                    if (checkboxSelections != null && checkboxSelections.Any())
                    {
                        foreach (var transc in checkboxSelections)
                        {
                            ...
                            var selectedOptions = transc.Options?.Where(o => o.Value == "true").ToList() ?? new List<OptionData>();
                            ...
                            if (transValue != null)
                            {
                                ...
                                foreach (var option in selectedOptions)
                                {
                                    repository.Add(new IncubationWorkshopOptionValues
                                    {
                                        ValueID = Guid.NewGuid(),
                                        TransID = transID,
                                        TransValueID = transValue.TransValueID,
                                ...
                                transValue.Value = selectedOptions.Count.ToString();
                                transValue.SubmissionDate = DateTime.UtcNow;
                                transValue.IncubationWorkshopBLTransValStatusID =
                                    repository.Get<IncubationWorkshopBLTransValStatus>(s => s.NameEN == "Post Impact Updated")
                                    .FirstOrDefault()?.IncubationWorkshopBLTransValStatusID
                                    ?? repository.Get<IncubationWorkshopBLTransValStatus>(s => s.NameEN == "Pending")
                                       .FirstOrDefault().IncubationWorkshopBLTransValStatusID;
                            }
                            else
                            {
                                var newTransValue = new IncubationWorkshopBLTransactionsValue
                                {
                                    TransValueID = Guid.NewGuid(),
                                    TransID = transID,
                                    Value = selectedOptions.Count.ToString(),
                                    SubmissionDate = DateTime.UtcNow,
                                    FrontendUserID = frontEnd,
                                    IncubationWorkshopBLTransValStatusID =
                                        repository.Get<IncubationWorkshopBLTransValStatus>(s => s.NameEN == "Pending")
                ...
                foreach (var key in Request.Form.AllKeys.Where(k => k.StartsWith("SelectedOptions") && !k.Contains("][")))
                {
                    var value = Request.Form[key];
                    if (!string.IsNullOrEmpty(value))
                    {
                        var transID = Guid.Parse(key.Split('[')[1].Split(']')[0]);
                        var selectedOptionID = Guid.Parse(value);
                        radioSelections[transID] = selectedOptionID;
                    }
                }
                ...
                if (radioSelections.Any())
                {
                    foreach (var radioSelection in radioSelections)
                    {
                        var transID = radioSelection.Key;
                        var selectedOptionID = radioSelection.Value;
                ...
                // 3) Text/Number/TextArea inputs
             
                var formKeys = Request.Form.AllKeys
                    .Where(f => f != "WorkshopName" && f != "WorkshopID" && !Request.Files.AllKeys.Contains(f.Split('_')[0]))
                    .ToList();

                foreach (var key in formKeys)
                {
                    var trans = transType.IncubationWorkshopBLTrans?.FirstOrDefault(f =>
                        f.FieldNameEn.Trim().Replace(" ", string.Empty) == key);

                    if (trans == null) continue;

                    var transValue = repository.GetQuery<IncubationWorkshopBLTransactionsValue>()
                        .FirstOrDefault(v =>
                            v.TransID == trans.TransID
                            && v.FrontendUserID == frontEnd
                            && v.IncubationWorkshopBLTrans.IncubationWorkshopID == WorkshopID
                            && v.IncubationWorkshopBLTrans.TransTypeID == PostImpactTransTypeId);

                    // تجاهل checkbox هنا (لأنه يجي من CheckboxData)
                    if (trans.IncubationWSControlsType?.IncubationWorkshopControl?.ControlsName == "Checkbox")
                        continue;
                    ...
                    {
                        repository.Add(new IncubationWorkshopBLTransactionsValue
                        {
                            TransValueID = Guid.NewGuid(),
                            TransID = trans.TransID,
                            Value = Request.Form[key],
                ...
                // 4) Files - باستخدام SavePostImpactFile
                ...
                foreach (var key in Request.Files.AllKeys.Distinct())
                {
                    ...
                    if (transValue == null)
                    {
                        transValue = new IncubationWorkshopBLTransactionsValue
                        {
                            TransValueID = Guid.NewGuid(),
                            TransID = trans.TransID,
                            Value = "",
                        ...
                        repository.Add(transValue);
                        repository.UnitOfWork.SaveChanges();
                    }

                    foreach (var file in reqfiles)
                    {
                        // حفظ الملف في فولدر PostImpact
                        var url = SavePostImpactFile(file, WorkshopID, transValue.TransValueID);

                        // تخزين مرفق
                        transValue.IncubationWSBLTransValueAttachment.Add(new IncubationWorkshopBLTransValueAttachment
                        {
                            TransAttachmentID = Guid.NewGuid(),
                            Name = Path.GetFileName(file.FileName),
                            ScreenName = "IncWSPostImpactForm",
                    ...
                    transValue.Value = $"{transValue.IncubationWSBLTransValueAttachment.Count} Attached File";
                    transValue.SubmissionDate = DateTime.UtcNow;
                    transValue.IncubationWorkshopBLTransValStatusID =
                        repository.Get<IncubationWorkshopBLTransValStatus>(s => s.NameEN == "Post Impact Updated")
                        .FirstOrDefault()?.IncubationWorkshopBLTransValStatusID
                ...
                // 5) تحديث كل سجلات الورشة للحالة النهائية
                ...
                var allPostImpact = repository.GetQuery<IncubationWorkshopBLTransactionsValue>()
                    .Where(v => v.FrontendUserID == frontEnd
                        && v.IncubationWorkshopBLTrans.IncubationWorkshopID == WorkshopID
                        && v.IncubationWorkshopBLTrans.TransTypeID == PostImpactTransTypeId)
                    .ToList();

                allPostImpact.ForEach(v =>
                {
                    v.SubmissionDate = DateTime.UtcNow;
                    v.IncubationWorkshopBLTransValStatusID = updatedStatus;
                });

                repository.UnitOfWork.SaveChanges();
             
                var request = repository.GetQuery<ProjectImpactEvaluationRequest>()
                    .Where(r => r.FrontendUserId == frontEnd &&
                                r.ProjectImpactEvaluation.IncubationWorkshopID == WorkshopID)
                    .Select(r => new { r.ProjectImpactEvaluationId })
                    .FirstOrDefault();

                if (request != null)
                {
                    string markMsg;
                    bool marked = TryMarkFilled(request.ProjectImpactEvaluationId, frontEnd, out markMsg);
                    //  6) إرسال بريد شكر للجمعية بعد إتمام قياس الأثر
                    // بعد نجاح الحفظ النهائي (آخر SaveChanges)
                    if (marked)
                    {
                        SendCompletionEmail(WorkshopID, frontEnd);
                    }
                }


                // TempData["Msg1"] = "تم حفظ تقييم الأثر بنجاح.";
        ...
        private void SendCompletionEmail(Guid workshopId, Guid frontEndUserId)
        {
            // جلب بيانات المستخدم (FrontendUser) مع AspNetUser + CorporateApplicationForm  
            var user = repository.GetQuery<FrontendUser>()
                .Include(f => f.AspNetUser)
                .Include(f => f.CorporateApplicationForms)
                .FirstOrDefault(f => f.UserID == frontEndUserId);

            var email = user?.AspNetUser?.Email;
            if (string.IsNullOrWhiteSpace(email)) return;

          
            var orgName =
                user.CorporateApplicationForms?.FirstOrDefault()?.Name
                ?? user.AspNetUser?.UserName
                ?? "منظمتكم";

            // جلب الورشة + FundingSource
            var workshop = repository.GetQuery<IncubationWorkshop>()
                .Include(w => w.FundingSource)
                .FirstOrDefault(w => w.IncubationWorkshopID == workshopId);

            if (workshop == null) return;

            var workshopName = !string.IsNullOrWhiteSpace(workshop.Name) ? workshop.Name : "المشروع";
            ...
            var mailHelper = new MailHelper
            {
                ...
                IsHtml = true,
                Body =
                    $"السادة/ {orgName} الموقرين<br/>" +
                    "السلام عليكم ورحمة الله وبركاته،،<br/><br/>" +
                    $"تهديكم مؤسسة الملك خالد أطيب التحيات، ونشكركم على إتمام استمارة قياس الأثر لمشروع {workshopName} " +
        ...
        [HttpGet]
        public FileResult DownloadPostImpactAttachment(Guid id)
        {
            try
            {
                ...
                var attachment = repository.GetByKey<IncubationWorkshopBLTransValueAttachment>(id);
                if (attachment == null)
                    return File(Encoding.UTF8.GetBytes("Attachment not found"), "text/plain", "error.txt");

                
                if (attachment.IncubationWSBLTransValue == null || attachment.IncubationWSBLTransValue.FrontendUserID != userId)
                    return File(Encoding.UTF8.GetBytes("Unauthorized"), "text/plain", "error.txt");

                var url = attachment.URL;

                
                string filePath = url.StartsWith("/")
                    ? Server.MapPath(url)     
                    : url;                    

                if (!System.IO.File.Exists(filePath))
                    return File(Encoding.UTF8.GetBytes("File not found"), "text/plain", "error.txt");
        ...
        private bool TryMarkFilled(Guid projectImpactEvaluationId, Guid frontendUserId, out string msg)
        {
            msg = string.Empty;
            ...
            try
            {
                var req = repository.GetQuery<ProjectImpactEvaluationRequest>()
                    .FirstOrDefault(r =>
                        r.ProjectImpactEvaluationId == projectImpactEvaluationId &&
                        r.FrontendUserId == frontendUserId);

                if (req == null)
                {
                    msg = "لم يتم العثور على طلب تقييم الأثر للمستخدم الحالي.";
                    return false;
                }

                // إذا كان معبّى سابقاً لا نعيد التحديث
                if (req.Status == ProjectImpactEvaluationStatus.Filled)
                {
                    if (!req.FilledOn.HasValue)
                        req.FilledOn = DateTime.UtcNow; // احتياط لو كانت null

                    msg = "الطلب مسبقاً بحالة Filled.";
                    repository.UnitOfWork.SaveChanges();
                    return true;
                }

                // تحديث للحالة النهائية
                req.Status = ProjectImpactEvaluationStatus.Filled;
                req.FilledOn = DateTime.UtcNow;

                repository.UnitOfWork.SaveChanges();

                msg = "تم تحديث حالة الطلب إلى Filled بنجاح.";
                return true;
            }
            catch (Exception ex)
            {
                msg = "حدث خطأ أثناء تحديث حالة الطلب: " + ex.Message;
                return false;
            }
        }
