...
namespace AlphaPeople.Repository
{
    public partial class Repository : IRepository, IDisposable
    {
        ...
        public List<TEntity> Get<TEntity>(Expression<Func<TEntity, bool>> whereExpression) where TEntity : class
        {
            if (whereExpression == null) throw new ArgumentNullException("whereExpression is null");
            IQueryable<TEntity> _Query = GetQuery<TEntity>().Where(whereExpression);
