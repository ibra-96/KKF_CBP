...
namespace AlfaPeople.KingKhaledFoundation.Admin.Web.Controllers
{
    ...
    public class CorporationRegisteredController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            var LstFrontendUserNotApproved = (User.IsInRole("Admin") && string.IsNullOrWhiteSpace(ProgramType)) ?
                repository.Find<CorporateApplicationForm>(f => f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Pending"&& f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Rejected").ToList() :
                repository.Find<CorporateApplicationForm>(f => (f.Program.ProgramName == ProgramType) && f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Pending" && f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Rejected").ToList();
        ...
        public ActionResult RejectedIndex()
        {
            ...
            var LstFrontendUserNotApproved = (User.IsInRole("Admin") && string.IsNullOrWhiteSpace(ProgramType)) ?
                repository.Find<CorporateApplicationForm>(f => f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Pending" &&f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Accepted").ToList() :
                repository.Find<CorporateApplicationForm>(f => (f.Program.ProgramName == ProgramType) && f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Pending" &&f.CorporateApplicationStatu.ApplicantStatu.ApplicantStatusName != "Accepted").ToList();
            //return View(LstFrontendUserNotApproved.OrderByDescending(o => o.CorporateApplicationStatu.DateTimeMakeAction));
            return View(LstFrontendUserNotApproved
            .Where(o => o.CorporateApplicationStatu != null)
                   .OrderByDescending(o => o.CorporateApplicationStatu.DateTimeMakeAction));
        ...
        public ActionResult ExportToExcel()
        {
            ...
            var data = (from caf in repository.GetAll<CorporateApplicationForm>()
                        ...
                        where as1 != null &&
                   (as1.ApplicantStatusName == "Accepted")
        ...
        public JsonResult AdminUpdateBackendUserAccount(string AspUserId, string UserName, string Email, string NewPassword)
        {
            ...
            // تحقق تكرار UserName
            var uByName = userManager.FindByName(newUserName);
            if (uByName != null && uByName.Id != user.Id)
                return Json(new { Success = false, Message = "اسم المستخدم موجود بالفعل." });

            // تحقق تكرار Email
            var uByEmail = userManager.FindByEmail(newEmail);
            if (uByEmail != null && uByEmail.Id != user.Id)
                return Json(new { Success = false, Message = "البريد الإلكتروني موجود بالفعل." });

            // تحديث UserName/Email
            user.UserName = newUserName;
            user.Email = newEmail;

            var updateRes = userManager.Update(user);
            if (!updateRes.Succeeded)
                return Json(new { Success = false, Message = string.Join(" | ", updateRes.Errors) });

            //  تحديث كلمة المرور + تحديث FrontendUsers.Password فقط عند وجود NewPassword
            if (!string.IsNullOrWhiteSpace(NewPassword))
            {
                user.PasswordHash = userManager.PasswordHasher.HashPassword(NewPassword);
                user.SecurityStamp = Guid.NewGuid().ToString();

                var passRes = userManager.Update(user);
                if (!passRes.Succeeded)
                    return Json(new { Success = false, Message = string.Join(" | ", passRes.Errors) });

                var feUser = repository.GetQuery<FrontendUser>(f => f.FK_AspUser == AspUserId).FirstOrDefault();
                if (feUser != null)
                {
                    feUser.Password = NewPassword;  
                    repository.Update(feUser);
                }

                repository.UnitOfWork.SaveChanges();
            }

            return Json(new { Success = true, Message = "تم تحديث بيانات الدخول بنجاح." });
        }
        ...
        public ActionResult ExportToExcel1()
    {
        ...
        var data = (from caf in repository.GetAll<CorporateApplicationForm>()
                    ...
                    where as1 != null &&
                      (as1.ApplicantStatusName == "Rejected")
