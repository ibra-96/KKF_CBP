...
namespace AlfaPeople.KingKhaledFoundation.Admin.Web.Controllers
{
    ...
    public class IncubationFollowUpEvaluationController : BaseController
    {
        ...
        public ActionResult Create(Guid? id, string Type)
        {
            ...
            var programId = repository.GetQuery<Program>(r => r.ProgramName == "Capacity Building")
                           .Select(r => r.ProgramID)
                           .FirstOrDefault();

            var corporations = repository.GetQuery<FrontendUser>(f =>
                    f.IsActive == true &&
                    f.CorporateApplicationForms.Any(c => c.ProgramID == programId))
                .ToList();

            var items = corporations.Select(u =>
            {
                var orgName = (u.CorporateApplicationForms != null && u.CorporateApplicationForms.Any())
                    ? (u.CorporateApplicationForms.FirstOrDefault().Name ?? "")
                    : "";

                var userName = (u.AspNetUser != null) ? (u.AspNetUser.UserName ?? "") : "";

                var text = !string.IsNullOrWhiteSpace(orgName)
                    ? $"{orgName} - ({userName})"
                    : userName;

                return new { u.UserID, Text = text };
            }).ToList();

            ViewBag.CorporationUserID = new SelectList(items, "UserID", "Text");
           //3-2-2026
